...
os.environ["R"] = "/home/claude/SR835_firmware/selache"
...
SUBMIT_PY = "$R/../test_serv/submit.py"
...
LEDGER = os.path.expandvars(f"{XTEST}/ledger.txt")
...
def build_target_objs():
    ...
    for src in c_sources:
        ...
        if (not os.path.exists(obj)
                or os.path.getmtime(obj) < os.path.getmtime(src)):
            sh(f"cc21k {CFLAGS} -c -o {obj_cmd} {src_cmd}")
        objs.append(obj_cmd)
