...
Demonstrate FPGA→STM32MP135 QSPI quad-output transfer of arbitrary data
at **≥400 Mbps wall rate with ZERO bit errors verified end-to-end over
...
bounded above by 100% (we only verify 0..firsterr−1). Validate by
...
vary across boots → SI metastability (attack via Step 5/6); (b)
...
At one presc step faster than the post-Step-5 ceiling, sweep DLYB tap ∈
{0, 8, 16, 32, 48, 64, 96, 128} × sshift ∈ {0, 1} × dummy_cycles ∈ {8,
...
asserting both `total_errors=0` and `mbps≥400`. Confirm `quadspi.bin`
reproduces the Step-7 result bit-for-bit.

## Step 9 — Add slave-side end-to-end CRC32 to `quadspi.nw`
...
asserts equality. This closes the "trust the receiver-side check" gap —
both sides must agree.

## Step 10 — Switch from address ramp to PRBS-15 in `quadspi.nw`
...
longer needs to know the expected pattern — it just compares the two
...
anymore — just CRC equality), wall rate ≥400 Mbps, single run. Once
...
(append one line per landed milestone here, format: `YYYY-MM-DD step N —
outcome — chapters now deletable: <list>`)
